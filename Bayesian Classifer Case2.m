...
%Reading in the training set
    ...
    train2= fopen('C:/TakeHome1/data/train_case_2.dat');
    training_set = cell2mat(textscan(train2, '%f %f %f %f'));
    fclose(train2);

%Reading in the test set

    test2 = fopen('C:/TakeHome1/data/test_case_2.dat');
    test_set = cell2mat(textscan(test2, '%f %f %f %f'));
    fclose(test2);
    
%Writing Classification Results to the file 
fileID = fopen('C:/TakeHome1/takehome1_case_2.txt','wt');    
example = fopen('C:/TakeHome1/exam.txt','wt');
fprintf(example,'Case2:\n');    

%Assigning Apriori Probabilities of each class
    Pw1=1/2;
    Pw2=1/3;
    Pw3=1/6;
...
%Training set classification
    for i=1:15000
        x=training_set(i,:);
        x=transpose(x);
        g1(i)= -0.5*(transpose(x-u1)*inv(sigma1)*(x-u1))-(0.5*log(det(sigma1)))+log(Pw1);
        g2(i)= -0.5*(transpose(x-u2)*inv(sigma2)*(x-u2))-(0.5*log(det(sigma2)))+log(Pw2);
        g3(i)= -0.5*(transpose(x-u3)*inv(sigma3)*(x-u3))-(0.5*log(det(sigma3)))+log(Pw3);
        %Classification Rule
...
%Classification Results for test_case_2  
